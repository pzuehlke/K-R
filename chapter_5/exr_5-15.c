...
int main(int argc, char* argv[])
{
    ...
    int fold = 0;       /* 1 to fold upper and lower case together */
    int c = 0;
    ...
    if ((nlines = read_lines(line_pointer, MAXLINES)) >= 0) {
        /* convert everything to lowercase if `fold` */
        if (fold) {
            for (int i = 0; line_pointer[i] != NULL; ++i) {
                to_lowercase(line_pointer[i]);
            }
        }
...
/* to_lowercase: convert an entire string to lowercase */
void to_lowercase(char str[]) {
